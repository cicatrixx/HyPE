function runname= runHydrus(runname_prefix, rootpath,...
    scenario, policytype, runsustain, geohazard_scentype, geohazard_select,...
    makechanges2main, makechanges2cost,futQpath)
...
% Optional inputs:
    ...
    % futQpath         - full path to mat file w futQ design flow
    % parameters
   

% Todo wishlist:
% - save Q_RD_design_mean in output 
% - get subbasin id and save the info, current RDRegion_id saves subbasin info but only for the initial shortlisting of outlets and not for inlets found later.
% - track both inlet and outlets for DP - currently for DP only inlets are saved
% - track whether DP is setup as small or large DP

%RP or RD=River power Plants and DP=Diversion canal plants
tic
close all
...
nbasin = 103; % basin file number to load
savedata=1;
showcheckplots=0;

% vars to change for sensitivity analysis: 
%Main function: sel_design_flows, storage_lim, Plarge_minH, Psmall_minH=20, eflow
...
elseif policytype==4 % MIXED FOCUS
    ...
    minDis_main_km=1;           %Minimum distance between cascading plants. applied to primary and secondary
...
if runsustain
    ...
    eflow_constraint=1; %Enable eflow of x% to ensure natural flow of the river (see eflow -setting)
    protarea_constr     =1; %Apply no dams in protected areas
else
    waterconsumption    =0; eflow_constraint=0;
    protarea_constr     =0;
...
geohazard_level_rates =[0	0  % Added percentage cost for each hazard level 1=Very Low, 5=Very High, percentage is divided by 100 in costmodel prior to use
    1	2
    2	4
    3	6
    4	8
...
if exist('futQpath','var')
    fprintf("Loading futQ for: %s\n", futQpath)
    clearvars Q*
    load(futQpath)
end
...
fprintf('Eflow constraint: %d%%\n', eflow_constraint*eflow*100);
...
disp("Following changes passed to mainconfig:")
disp(makechanges2main)
disp("Following changes passed to costconfig:")
disp(makechanges2cost)
...
if runsustain
    if geohazard_scentype==0  % No geohazard
        geohazard_cost      =0;
        geohazard_constr    =0;
    elseif geohazard_scentype==1 % Cost-based Status Quo 
...
elseif geohazard_cost && geohazard_constr ==2 %% for multi-hazard scenario
    % Composite considering sum of all 
    geohazard_sum_naned=maskBasin(sum(geohazard_compile,3), ~outside);
    
    % hazard rescaled to 0-5 from 0-20 
...
%% Eflow for ecological reasons - not applied on smallP
if eflow_constraint==1
    Q_RD_design = Q_RD_design *( 1-eflow);
    Q_P_design = Q_P_design *( 1-eflow);
    Q_P_design_mean = Q_P_design_mean *( 1-eflow);
end
    ...
    for ndl = 1:nd
        ...
        for k = 1:numel(outlets)
            ...
            for l = 1:numel(ZPin)
                ...
                %
                %                 if eflow_constraint==1
                %                     QDesignPinlet{k}{l} = QDesignPinlet{k}{l} *( 1-eflow);
                %                 end
                %
                %Check
        ...
        %% Determine lat lon coordinates of outlets and inlets
        disp('Calculate pipelength and lat lon');
        ...
        for k= 1:n_outlets
            ...
            dfPLmin(k)= PL{k}{a1(k)}(a2(k)); %Pipelength
            HeadPmin(k) = ZPinlet{k}{a1(k)}(a2(k)) - Zoutlets(k);  %Hgross
            HeadraceLPmin(k)= PL{k}{a1(k)}(a2(k)) - (ZPinlet{k}{a1(k)}(a2(k)) - Zoutlets(k));
            aPInletMin(k) = Pinlet{k}{a1(k)}(a2(k));  %best inlet index
            ...
            OpthfPMin(k) = OpthfP{k}{a1(k)}(a2(k)); % Percentage of Hgross lost due to head loss
        ...
        dfPLminEnd{ndl}=dfPLmin; % Pipe length
        CostElementsPMinEnd{ndl}=CostElementsPMin;
        Pinlet_winend{ndl}=Pinlet_win;
        HeadPminend{ndl}=HeadPmin;  %Optimal gross head
        ...
        ZPinletMinend{ndl} = ZPinletMin; % Optimal Zin for inlet
        nPipePMinend{ndl} = nPipePMin; %optimal number of pipes
        ...
        OpthfPminend{ndl} = OpthfPMin;  %Optimal friction loss in terms of percentage of Hgross lost due to head loss
...
fprintf('%s Financial Potential: %0.2f GWh\n',scenario, sum(PnetAlls(COEAlls<=0.1),'omitnan'))
...
grid on
title(sprintf('Evaluated and optimal projects in %s\nTechnical Potential: %0.2f TWh \nFinancial Potential: %0.2f TWh',runname,sum(PnetAlls,'omitnan')/1000,sum(PnetAlls(COEAlls<=0.1)/1000),'omitnan'),'Interpreter','none')
